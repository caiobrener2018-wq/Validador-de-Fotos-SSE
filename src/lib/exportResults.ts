...
export function exportResultsToExcel(agents: AgentData[]) {
  const rows: any[][] = [
    ['Linha Excel', 'Agente', 'Empresa', 'Segmento', 'Planilha',
  ...
  for (const agent of agents) {
    const row: any[] = [agent.excelRow, agent.name, agent.companyName, agent.segment, agent.sourceFile];
    for (let i = 0; i < 3; i++) {
  ...
  ws['!cols'] = [
    { wch: 10 }, { wch: 30 }, { wch: 30 }, { wch: 20 }, { wch: 25 },
